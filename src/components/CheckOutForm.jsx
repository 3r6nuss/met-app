...
import { PackageMinus, DollarSign, Copy, Check, ShoppingCart, Trash2, Sparkles, Plus } from 'lucide-react';
...
export default function CheckOutForm({
    ...
    const accentFrom = isVerkauf ? 'from-amber-500' : 'from-orange-500';
    const accentTo = isVerkauf ? 'to-orange-400' : 'to-red-400';
    const accentText = isVerkauf ? 'text-amber-400' : 'text-orange-400';
    const accentBorder = isVerkauf ? 'border-amber-500/30' : 'border-orange-500/30';
    const summaryAccent = isVerkauf ? 'text-amber-400' : 'text-orange-400';
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className={`grid ${showPrice ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                                ...
                                {showPrice && (
                                    <div className="space-y-1.5">
                                        <label className="text-xs text-muted-foreground uppercase tracking-widest font-semibold">Preis (Stk)</label>
                                        <div className="relative">
                                            <Input type="text" value={price}
                                                onChange={(e) => setPrice(e.target.value)}
                                                placeholder="0"
                                                className="bg-slate-950/60 border-slate-700/60 text-slate-200 pl-8" />
